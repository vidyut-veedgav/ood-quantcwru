...
def load_psm_processed(processed_path, subset):
    if subset == "train":
        data = np.load(os.path.join(processed_path, "train.npy"))
        print ("in load: ", os.path.join(processed_path, "train.npy"))
        ...
        print ("in load: ", os.path.join(processed_path, "test.npy"))
        print ("in load: ", os.path.join(processed_path, "test_labels.npy"))
    else:
...
class PSMDataset(Dataset):
    ...
    def __init__(
        ...
        if use_processed:
            ...
            data, labels = load_psm_processed(processed_path, subset)

            print("Processed path:", processed_path)
            print("Full label path:", os.path.join(processed_path, "test_labels.npy"))

            self.post_scaler = None  # 🚨 IMPORTANT: no scaling
            self.data = data
            self.labels = labels
            print ("-------LABELS------------")
            ...
            data_path = os.path.join(data_path, file_map[subset])
            ...
            if subset == "test":
                # label_file = os.path.join(data_path, "test_label.csv")
                base_dir = os.path.dirname(data_path)
                label_file = os.path.join(base_dir, "test_label.csv")
